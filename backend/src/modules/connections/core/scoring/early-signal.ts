...
export function computeEarlySignal(input: EarlySignalInput): EarlySignalResult {
  ...
  if (input.trend.acceleration_norm > 0.3) {
    reasons.push('Ускорение роста выше среднего')
  }
  
  if (relativeGap > 0.2) {
    reasons.push('Тренд существенно усилил влияние')
  }
  
  if (input.profile === 'retail') {
    reasons.push('Ранний аккаунт — потенциал роста выше')
  }
  
  if (input.profile === 'whale') {
    reasons.push('Крупный аккаунт — меньше потенциала для "раннего" роста')
  }
  
  if (input.risk_level !== 'low') {
    reasons.push('Присутствуют риски, сигнал ослаблен')
  }
  
  if (growthPressure > 0.5) {
    reasons.push('Высокое давление роста')
  }
